...
export async function matchCardImage(photo: Blob, topN = 8): Promise<HashMatch[]> {
  ...
  // crop to the detected card; fall back to the full frame
  ...
  const r = found
    ? {
        x: found.rect.x0 / scale,
        y: found.rect.y0 / scale,
        w: (found.rect.x1 - found.rect.x0) / scale,
        h: (found.rect.y1 - found.rect.y0) / scale,
      }
    : { x: 0, y: 0, w: bitmap.width, h: bitmap.height }

  const probe = hashCanvas(bitmap, r.x, r.y, r.w, r.h)
  bitmap.close()
